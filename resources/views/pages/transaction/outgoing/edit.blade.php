...
@section('content')
    ...
    <div class="card mb-4">
        <div class="card-header">
            <h5 class="mb-0">Formulir Edit Surat Keluar</h5>
            <small class="text-muted">Silakan perbarui data surat di bawah ini.</small>
        </div>
        ...
        <form action="{{ route('transaction.outgoing.update', $data) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="card-body">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <x-input-form :value="$data->to" name="to" :label="__('model.letter.to')" placeholder="Tujuan surat (Instansi/Perorangan)"/>
                    </div>
                    <div class="col-md-6">
                        <x-input-form :value="$data->note ?? ''" name="note" :label="__('model.letter.note')" placeholder="Catatan tambahan (Opsional)"/>
                    </div>
                    <div class="col-12">
...
@push('script')
    <script>
        // SweetAlert untuk Konfirmasi Hapus Lampiran
